...
Start-Transcript -Path "$Path_local\Log\OneDriveDownloads-install.log" -Force -Append
...
#half-ass hack to make sure it's not currently in the onedrive folder
#replace with your org config
if ($downloadsPath -like "*OneDrive - JCS Solutions LLC*") {
    Write-Host "Downloads path is correctly configured in OneDrive"
} else {
    #set new directory in onedrive
    $newDownloads = "C:\Users\$user\OneDrive - JCS Solutions LLC\Downloads"

    #create directory in OneDrive if it does not exist
    if (!(Test-Path $newDownloads)) {
        #make the folder if it does not exist
        New-Item -Path $newDownloads -ItemType Directory
    }

    #move current downloads folder to new path
    Get-ChildItem $downloadsPath -Recurse | Move-Item -Destination $newDownloads

    #set folder location
    Set-KnownFolderPath -KnownFolder 'Downloads' -Path $newDownloads
}
